...
static int show_hint(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    printf("kernel7 is the original Raspbian image\n"
	   "For some reason, to use the initramfs, we must first load it, then the kernel, then the flattened device tree. Eg) run loadInitramfs; run loadKernel; run loadFdt; run loadEnv; run doBoot2\n"
